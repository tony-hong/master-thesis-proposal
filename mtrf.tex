...
\begin{aligned}
    \{ 0:9527,\ 1:20,\ 2:50,\ 3:0,\ 4:0,\ 5:4311,\ 6:50001,\ 7:0\}
\end{aligned}
\end{equation}


\newpage
\section{Multi-task Event Predicate-Participants Model} \label{sec:mtrf}
...
\begin{aligned}
    \mathcal{L}(\theta, e)
        &= \mathcal{L}^{(pr)}(\theta, e) \mathcal{L}^{(pa)}(\theta, e) + \alpha \mathcal{L}^{(sr)}(\theta, e)
\end{aligned}
\end{equation}
where $\theta$ representing all parameters in the model and $\alpha$ is the hyper-parameter for the weight of semantic role prediction task. Combining with objectives we define above, the likelihood function can be written as: 
\begin{equation} \label{eq:likelihood-obj}
\begin{aligned}
    \mathcal{L}(\theta, e)
        &= O^{(pr)}(pr) \prod_{i \in L-1} O^{(pa)}(i) + \alpha \prod_{j \in L} O^{(sr)}(j) 
\end{aligned}
\end{equation}
where $L$ is the number of all non-empty head words in the event. So $L-1$ is the number of event participants (where the predicate is not included). The model is trained over all events in the training data to obtain parameters $\theta$ that maximize the likelihood function as:
...
To perform maximum likelihood estimation, we process a fixed number of input entries, a batch, each time and use AdaGrad algorithm \citep{duchi2011adaptive} as optimizer to update parameters. The batch size $bs$ and the learning rate $lr$ are hyper-parameters. When half of the event predicates and participants in training data are processed one time, the period is considered as an epoch. After each training epoch, the model is validated on the validation data. The validation loss is defined as: 
...
The hyper-parameters for model training we use are listed in Table \ref{tab:hyper}. We firstly use same hyper-parameter setting as \citet{tilk2016event} to make a fair comparison. We choose $1.0$ as the weight of semantic role prediction for simplicity. Because the data scale is enormous, in order to speed-up the training process, we make use of all 12 GB video memory and set the $bs$ to $4000$. Using this batch size, we perform feature selection on learning rate. We train MTEP on the first $10\%$ of training set for $5$ epochs and observe the perplexity on test data. We compare the values in the set of $\{0.05, 0.1, 0.15\}$. The setting of $0.05$ has worst performance and the setting of $0.15$ causes gradient explosion from time to time. The experiments show that $0.1$ is the optimised setting. We adopt the setting of $es$ from \citet{tilk2016event}, set $dp$ as $1$ epoch smaller than $es$ and pick $dr$ according to the suggestion of \citet{he2016deep}. 
...
$k$         &   Embeddings cardinality      &   256     \\
$d$         &   Hidden layer cardinality    &   512     \\
$k^{(w)}$   &   Word output cardinality &   512     \\
$k^{(w)}$   &   Role output cardinality &   512     \\
$\alpha$    &   Role prediction weight  &   1.0     \\  \hline
$bs$        &   Batch size              &   4000    \\
...
\subsection{Evaluation} \label{sec:comp_mtrf}
In order to show whether the event embedding similarity problem is solved, we perform a model comparison between NNRF and MTEP. Two models are implemented with the same number of hyper-parameters, except MTEP has additional parameters in the target role classifier. We trained and validated both models on the RW-eng corpus. 
...
\begin{tabular}{l|ll|l}
    ...
    \textbf{MTEP}   &   68 $\pm 13$ &   52 $\pm 14$ &   \textbf{16}  \\
\end{tabular}
...
\caption{\label{fig:GS-NNRF-MTEP} Event similarity evaluation by training epoch of NNRF and MTEP. $x$-axis is the number of training epochs. $y$-axis is the Spearman's rho between model scores andhuman judgements. }
\end{figure}
...
\textbf{Characteristics}    & \textbf{NNRF} & \textbf{MTEP} & \textbf{BOP} & \textbf{BOP Res} \\ \hline
...
Dataset &   TypeDM  &   SDDM    &   NNRF    &   MTEP    &   BOP &   BOP Res     &   BOP Res*    \\ \hline
...
We choose the result at the $30$-th epoch as the final result and compare these two systems with previous state-of-the-art models. The result is in Table \ref{tab:GS-mtrf}. 


\begin{table}[t]
\centering
\begin{tabular}{l||lll|ll|l|l}
\textbf{Model}          &   Add & Multiply & Kronecker & CBOW &   NNRF    &   MTEP  &   Human   \\ \hline
\textbf{$\rho$ * 100}   &   10  &   16  &   26          &   13      &   32 (34) &   \textbf{35} &   62      \\
\end{tabular}
\caption{\label{tab:GS-mtrf} Event similarity evaluation comparing to previous models. $p\text{-value} < 0.05$ for all results. }
\end{table}
...
Human       &       -   &   -       &   -       &   67  \\
\end{tabular}
\caption{\label{tab:wordsim} Word similarity evaluation on SimLex999 comparing to previous models. Each score is Spearman's $\rho$ * 100. $p\text{-value} < 0.05$ for all results. SOA is the state-of-the-art model. }
\end{table}
